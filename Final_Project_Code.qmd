...
As you can see in the data the when we trty to fit a linear line through the data it does not fit well. Thus we decided to transform the data which after testing, we decided to transform the mean Daily Income for each country using a natural logrithmic transformation. Below is the scatter plot and linear fit of this transformed data.
...
Based on the table above the regression equation is: ${\hat{HappinessScore}} = 31.59 + 8.73*log(DailyIncome)$

From this model we predict that a country with a log of their mean daily income equal to 0 will have a mean happiness score of 31.59 over the years 2005 to 2023. If we multiply the mean Daily Income by e (approximatly 2.718), we predict a increase of 8.73 in the mean Happiness Score for a country. This is a significant increase in the Happiness score since the value is a score between 1 and 100. Thus, we predict that whenever Daily Income increases we should see an increase in the Happiness score for a country. Below is a table showing the variance of our model which is helpful for analysis of the overall performance of our model.
...
As you can see from the table there is a lot of variation in the R-Squared for each model. Some are very high like folds 3 and 14 while some are very low like fold 10. Thus this varaition in the values means that sometimees the model is very good at predicting the countries happiness score and sometimes the trained model can not accuratly determine the countries happiness score from the daily income.In the graph below you can see the distribution of R-Squared compared to mean value.
...
The average R-squared values, shown by the blue line, is 0.5629. This means that 56.29% of the variability in Happiness Score can be explained by Daily Income. This is a very differnt prediction than the full model earlier which was 72.28%. However the average R-Squared is a much better predictor thus Daily Income is even less of a predictor of Happiness Score than we previosuly expected. Also now there is slightly more than 40% of the variance it still unexplained compared to around 30% before.
